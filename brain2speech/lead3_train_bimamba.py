#!/usr/bin/env python3
"""
Lead 3: BiMamba Decoder Training — Linderman Lab Reproduction.

Drop-in GRU replacement using bidirectional Mamba (Caduceus weight-tying).
Ref: arxiv 2412.17227 Section 2.1

Usage:
    # Basic BiMamba
    CUDA_VISIBLE_DEVICES=8 python brain2speech/lead3_train_bimamba.py \
        --experiment L3_mamba_base --n-layers 5 --d-state 16 \
        --d-model 512 --lr 0.02 --max-minibatches 10000 --seed 42

    # With Linderman enhancements
    CUDA_VISIBLE_DEVICES=8 python brain2speech/lead3_train_bimamba.py \
        --experiment L3_mamba_enhanced --n-layers 5 --d-state 16 \
        --speckled-mask 0.3 --fastemit 0.01 --post-backbone-norm \
        --lr 0.025 --scheduler linear --max-minibatches 20000 --seed 42

    # Separate (untied) forward/backward
    CUDA_VISIBLE_DEVICES=8 python brain2speech/lead3_train_bimamba.py \
        --experiment L3_mamba_sep --no-weight-tie --seed 42
"""
import argparse
import json
import math
...
from lead3_models import (
    BiMambaDecoder, supervised_contrastive_loss,
    fastemit_regularization, speckled_mask,
)


def evaluate_model(model, dataloader, device, kernel_size=14, stride=4):
    """Evaluate model, return PER and CTC loss."""
    model.eval()
    all_per = []
    total_loss = 0
    n_batches = 0
...
def train_epoch(model, dataloader, optimizer, criterion, scaler, device,
                ...
                speckled_mask_prob=0.0, fastemit_lambda=0.0,
                supcon_weight=0.0, supcon_temp=0.1):
    """Train one epoch with all Lead 3 enhancements."""
    model.train()
    total_loss = 0
    total_ctc = 0
    total_aux = 0
    ...
    for features, targets, feat_lens, tgt_lens, session_ids in dataloader:
        ...
        # Speckled masking
        if speckled_mask_prob > 0:
            features = speckled_mask(features, prob=speckled_mask_prob)
        ...
        with autocast("cuda"):
            ...
            if fastemit_lambda > 0:
                fe_loss = fastemit_regularization(log_probs, blank=CTC_BLANK,
                                                   lambda_fe=fastemit_lambda)
                loss = loss + fe_loss
                aux_loss = aux_loss + fe_loss
            ...
            if supcon_weight > 0:
                ...
                if hasattr(m, 'get_hidden'):
                    with autocast("cuda", enabled=False):
                        hidden = m.get_hidden(features.float(), session_ids)
                        # Create approximate per-frame labels by spreading
                        frame_labels = _spread_labels(
                            targets, tgt_lens, hidden.shape[1],
                            feat_lens, kernel_size, stride
                        )
                        if frame_labels is not None:
                            # Flatten batch and time dims
                            h_flat = hidden.reshape(-1, hidden.shape[-1])
                            l_flat = frame_labels.reshape(-1).to(device)
                            # Filter valid (non-blank) frames
                            valid = l_flat >= 0
                            if valid.sum() > 10:
                                sc_loss = supervised_contrastive_loss(
                                    h_flat[valid], l_flat[valid],
                                    temperature=supcon_temp
                                )
                                loss = loss + supcon_weight * sc_loss
                                aux_loss = aux_loss + supcon_weight * sc_loss
...
def _spread_labels(targets, tgt_lens, T_out, feat_lens, kernel_size, stride):
    """Spread phoneme labels uniformly across CTC output frames."""
    B = len(tgt_lens)
    labels = torch.full((B, T_out), -1, dtype=torch.long)
    offset = 0
    for i in range(B):
        P = tgt_lens[i].item()
        T = min((feat_lens[i].item() - kernel_size) // stride + 1, T_out)
        if P == 0 or T <= 0:
            offset += P
            continue
        t_per_phone = max(1, T // P)
        for p in range(P):
            t_start = p * t_per_phone
            t_end = min((p + 1) * t_per_phone, T)
            labels[i, t_start:t_end] = targets[offset + p].item()
        offset += P
    return labels


def main():
    parser = argparse.ArgumentParser(description='Lead 3: BiMamba Decoder')
    ...
    parser.add_argument('--speckled-mask', type=float, default=0.0,
                        help='Speckled masking probability (Linderman: 0.3)')
    parser.add_argument('--fastemit', type=float, default=0.0,
                        help='FastEmit lambda (Linderman: 0.01)')
    parser.add_argument('--supcon-weight', type=float, default=0.0,
                        help='supTCon loss weight (MONA LISA: 0.1)')
    parser.add_argument('--supcon-temp', type=float, default=0.1,
                        help='supTCon temperature')

    # Data augmentation
    parser.add_argument('--specaugment', action='store_true')
    parser.add_argument('--time-mask-max', type=int, default=25)
    parser.add_argument('--freq-mask-max', type=int, default=80)
    ...
    print('=' * 70)
    print(f'LEAD 3 — BiMamba: {args.experiment}')
    print(f'  d_model={args.d_model}, layers={args.n_layers}, '
          f'd_state={args.d_state}, expand={args.expand}')
    print(f'  Weight-tie: {args.weight_tie}, Post-backbone: {args.post_backbone_norm}')
    print(f'  LR: {args.lr}, Scheduler: {args.scheduler}')
    print(f'  Speckled mask: {args.speckled_mask}, FastEmit: {args.fastemit}')
    print(f'  supTCon: weight={args.supcon_weight}, temp={args.supcon_temp}')
    ...
    print(f'\nBiMambaDecoder: {n_params/1e6:.1f}M params')
    print(f'  weight_tie={args.weight_tie}, post_backbone={args.post_backbone_norm}')
    ...
    spec_aug = None
    if args.specaugment:
        from train_ctc_improved import NeuralSpecAugment
        spec_aug = NeuralSpecAugment(
            time_mask_max=args.time_mask_max,
            feat_mask_max=args.freq_mask_max,
        ).to(device)
    ...
    for epoch in range(total_epochs):
        ...
        train_metrics = train_epoch(
            ...
            kernel_size=args.kernel_size, stride=args.stride,
            speckled_mask_prob=args.speckled_mask,
        ...
        print(f'Epoch {epoch+1:3d}/{total_epochs} | '
              f'loss={train_metrics["ctc_loss"]:.4f} '
        ...
        history.append({
            ...
            'train_ctc': train_metrics['ctc_loss'],
    ...
    print(f'  Val CTC loss: {val_metrics["ctc_loss"]:.4f}')
    print(f'  Test PER:     {test_metrics["per"]:.1%}')
    print(f'  Test CTC:     {test_metrics["ctc_loss"]:.4f}')
    print(f'  Paper GRU:    19.7% PER (reference)')
    print(f'{"="*70}')
    ...
    save_path = RESULTS_DIR / f'L3_bimamba_{args.experiment}_{timestamp}.json'
    with open(save_path, 'w') as f:
        json.dump(results, f, indent=2)
    print(f'Results: {save_path}')

    model_path = RESULTS_DIR / f'L3_bimamba_{args.experiment}_best.pt'
    torch.save({
